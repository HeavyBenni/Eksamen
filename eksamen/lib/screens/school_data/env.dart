...
class EnvStaff extends StatefulWidget {
  const EnvStaff({Key? key});

  @override
  State<EnvStaff> createState() => _EnvStaffState();
}

class _EnvStaffState extends State<EnvStaff> {
  @override
                    ...
                    child: TextButton(
                        onPressed: () {
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (context) => const EnvStaffAdd(),
                            ),
                          );
                        },
                                      ...
                                      onPressed: () {
                                        try {
                                          ...
                                          ScaffoldMessenger.of(context)
                                            ...
                                            const SnackBar(
                                              content: Text(
                                                  'Miljøfagarbeider slettet'),
                                            ),
                                          );
                                          //Navigator.of(context).pop();
                                        } catch (error) {
                                          print('Error adding lærer: $error');
                                        }
                                      },
...
class EnvStaffAdd extends StatefulWidget {
  const EnvStaffAdd({super.key});

  @override
  State<EnvStaffAdd> createState() => _EnvStaffAddState();
}

class _EnvStaffAddState extends State<EnvStaffAdd> {
  ...
  Future<void> _saveData() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('Miljøfagarbeider').add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text('Legg til en ny Miljøfagarbeider'),
      ),
      body: Form(
        key: _formKey,
        child: Container(
                    ...
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        try {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Miljøfagarbeider lagt til'),
                            ),
                          );
                          Navigator.of(context).pop();
                        } catch (error) {
...
class EnvStaffEdit extends StatefulWidget {
  const EnvStaffEdit({Key? key});

  @override
  State<EnvStaffEdit> createState() => _EnvStaffEditState();
}

class _EnvStaffEditState extends State<EnvStaffEdit> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
